...
from gnn_disease.evaluation import get_disease_auc
...
class Trainer(object):
    def __init__(self, model, tau, optimizer, log_every_n_steps, device, name):
        ...
        self.writer.log_dir = "runs/" + name
        os.makedirs(self.writer.log_dir, exist_ok=True)
        logging.basicConfig(
            filename=osp.join(self.writer.log_dir, "training.log"), level=logging.DEBUG
        )
    ...
    def train(self, epochs):
        ...
        logging.info(
            f"Model checkpoint and metadata have been saved at {self.writer.log_dir}."
        )
        pickle.dump(
            g_h, open(osp.join(self.writer.log_dir, "gene_embedding.pkl"), "wb")
        )
