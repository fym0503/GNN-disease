...
def generate_inverses(triples, num_rels):
    ...
    inverse_relations = torch.cat(
        [triples[:, 2, None], triples[:, 1, None] + num_rels, triples[:, 0, None]],
        dim=1,
    )
...
def generate_self_loops(num_ents, num_rels, device="cpu"):
    ...
    id = torch.empty(size=(num_ents, 1), device=device, dtype=torch.long).fill_(
        2 * num_rels
    )
...
def get_kg_data(triples, num_rels):
    ...
    edge_index = torch.cat([triples[:, 0, None], triples[:, 2, None]], dim=1).permute(
        1, 0
    )
